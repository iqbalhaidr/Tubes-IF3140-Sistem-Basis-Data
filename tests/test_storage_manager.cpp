...
int main() {
    ...
    // Note: StorageEngine is a singleton, but we can't easily test with a custom directory
    // For now, use the default instance. If custom directory testing is needed,
    // we may need to add a reset method or use a different approach.
    auto& sm = StorageEngine::get_instance();
